...
(function(){
  ...
  function checkHoistableFunc(){
	...
	for(var i = 0; i<funcHoistbleMap.length;i++){
		...
		output +="\n Function ---> "+funcHoistbleMap[i].funcName +" under "+funcHoistbleMap[i].parentFunc+" can be hoisted";
		...
		//If this is the first entry of stack set parentFunc as Noparent
			if(funcNameStack.length === 0)
				funcHoistbleMap.push(
					{id: funcHoistbleMap.length,funcName: name,hoistable: "undefined",parentFunc: "NoParent",parentvar:
					     "",respAnscestor: ""});
			else
				funcHoistbleMap.push(
					{id: funcHoistbleMap.length,funcName: name,hoistable: "undefined",parentFunc: 
					    funcNameStack[funcNameStack.length-1],parentvar: "",respAnscestor: ""});
		}
	   }
	}
